...
const localCsvPath =
  "C:\\Users\\Wuttikorn\\Documents\\TLF\\forecast-app\\public\\ping_result.csv";
...
const parsePingCSV = () => {
  try {
    ...
    const parsedNodes = blocks.map((block) => {
      ...
      const [stadium, device] = desc.includes("#")
        ? desc.split("#")
        : ["ทั่วไป", desc];
      const status =
        getValue("Last Ping Status") === "Succeeded" ? "ONLINE" : "TIMEOUT";
...
app.post("/api/sync-ping", (req, res) => {
  if (Array.isArray(req.body)) {
    ...
    console.log(
      `[Cloud Sync via POST] ได้รับข้อมูลใหม่ ${req.body.length} รายการ และกระจายสัญญาณไปหา Frontend แล้ว`,
    );
    return res
      .status(200)
      .json({
...
setInterval(() => {
  ...
  if (
    isProduction &&
    globalPingData.length > 0 &&
    Date.now() - lastAgentSeen > STALE_TIMEOUT
  ) {
    console.log(`[Stale Detector] ไม่ได้รับข้อมูลจาก Agent เกิน 2 นาที!`);

    // --- ส่วนนี้คือจุดที่ทำให้หน้าจอกระพริบเป็นสีแดง ---
    // ให้คุณ "คอมเมนต์" (ใส่ // หน้าบรรทัด) หรือ "ลบ" 3 บรรทัดนี้ทิ้งได้เลยครับ:
    ...
    // แทนที่ด้วยการส่งแค่ Alert ไปบอกหน้าเว็บว่า Agent กำลังมีปัญหาแทน
    io.emit("agent-status", { message: "Agent Offline" });
  }
...
io.on("connection", (socket) => {
  ...
  socket.on("client-ping-sync", (data) => {
    if (Array.isArray(data)) {
      ...
      io.emit("ping-update", globalPingData);
      console.log(
        `[Socket Sync] ซิงค์ข้อมูลสดสำเร็จจำนวน ${data.length} รายการ จากคอมพิวเตอร์`,
      );
    }
  });
...
server.listen(PORT, () =>
  console.log(`Backend ระบบแชร์ข้อมูล Ping รันสำเร็จที่พอร์ต: ${PORT}`),
);
